...
```{r}
fam_file_A2 <- fread("./02_activities/data/gwa.A2.fam")
nrow(fam_file_A2)
# 4000 
print(paste0("There are ", nrow(fam_file_A2), " samples in the dataset"))
```
...
```{r}
bim_file_A2 <- fread("./02_activities/data/gwa.A2.bim")
nrow(bim_file_A2)
# 306102
print(paste0("There are ", nrow(bim_file_A2), " SNPs in the dataset"))
```
...
# Create new dataset with pruned SNPs
...
```{bash}
# Identify unrelated individuals up to 2nd degree relatives
plink2 --bfile ./02_activities/data/gwa.qc_pruned.A2 --king-cutoff 0.0884 --out ./02_activities/data/gwa.qc.A2
```
...
```{bash}
# Dataset containing only unrelated individuals
plink2 --bfile ./02_activities/data/gwa.qc.A2 --keep ./02_activities/data/gwa.qc.A2.king.cutoff.in.id --make-bed --out ./02_activities/data/gwa.qc_unrelated.A2
```
...
```{bash}
# Keep pruned SNPs in the unrelated dataset
...
# Perform PCA
plink2 --bfile ./02_activities/data/gwa.qc_unrelated_pruned.A2 --pca 20 --out ./02_activities/data/gwa_unrel_PCA_A2
...
```{bash}
plink2 --bfile ./02_activities/data/gwa.qc_unrelated.A2 --glm --covar ./02_activities/data/gwa_unrel_PCA_A2.eigenvec --covar-name PC1,PC2,PC3 --adjust --out ./02_activities/data/gwa.qc_assoc_A2
```
...
# Keep only the additive test (each SNP has one beta estimate/OR)
assoc_add <- assoc[TEST == "ADD"]
# Rename columns
setnames(assoc_add, c("#CHROM","POS","ID"), c("CHR","BP","SNP"))

# Make Manhanttan plot
png("./02_activities/assignments/assignment_2_manhattan_plot.png", width = 1200, height = 800, res = 150)
...
```{r}
# Q-Q plot
png("./02_activities/assignments/assignment_2_QQ_plot.png", width = 1200, height = 800, res = 150)
...
-   Confirm that both `assignment_2.qmd` and `assignment_2.html` and 2 figures are included in the pull request.
